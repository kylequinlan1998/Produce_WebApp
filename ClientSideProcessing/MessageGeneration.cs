	...
	public class MessageGeneration
	{
		...
		private ComputedDataModel GetSleepMessage(ComputedDataModel computedDataModel)
		{
			if (computedDataModel.SleepDeficit < 0)
			{
				...
				computedDataModel.SleepMessage += " You are currently operating on a sleep deficit of " + computedDataModel.SleepDeficit + " hours";
				return computedDataModel;
			}
		...
		{
			if(computedDataModel.BreaksDeficit < 0)
			{
				computedDataModel.BreaksMessage = "You have not taken enough Breaks, " + computedDataModel.Breaks + " Breaks can result in decreased " +
					" productivity and more physical pressure on your body.";
				return computedDataModel;
			}
			else if (computedDataModel.BreaksDeficit ==0)
			{
				computedDataModel.BreaksMessage = "You have had the correct number of Breaks. By taking " + computedDataModel.Breaks + " breaks You have increased efficiency and mood";
				return computedDataModel;
			}
			else
			{
